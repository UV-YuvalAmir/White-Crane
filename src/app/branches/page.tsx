        ...
        <div className="max-w-3xl mx-auto space-y-16">
          <div>
            ...
            <div className="border border-ink/5 overflow-hidden">
              <div className="hidden sm:grid grid-cols-3 bg-ink text-paper text-xs tracking-wide uppercase py-3 px-4">
                <span>{locale === "he" ? "יום" : "Day"}</span>
                <span>{locale === "he" ? "שעה" : "Time"}</span>
                <span>{locale === "he" ? "שיעור" : "Class"}</span>
              </div>
              {tlvSchedule.map((row, i) => (
                <div key={i} className={`sm:grid sm:grid-cols-3 py-3 px-4 text-sm border-b border-ink/5 ${i % 2 === 0 ? "bg-paper" : "bg-paper-dark"}`}>
                  <span className="text-ink font-medium text-xs block">{locale === "he" ? row.day : row.dayEn}</span>
                  <span className="text-stone-warm block">{row.time}</span>
                  <span className="text-stone-warm text-xs block">{locale === "he" ? row.classHe : row.classEn}</span>
                </div>
              ))}
            </div>
          </div>
